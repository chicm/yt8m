...
def convert_neg_train_data():
    try:
        os.stat(TRAIN_NEG_DIR)
    except:
    ...
    for fn in tqdm(train_files[:1000]):
        save_neg_train_data_as_npy(fn, TRAIN_NEG_DIR, meta)

    df_train = pd.DataFrame(meta)
    df_train = df_train[['vid', 'label', 'nframes']]
    df_train.to_csv('train_neg.csv', index=False)
...
def save_neg_train_data_as_npy(tf_filename, out_dir, meta):
    for example in tqdm(tf.python_io.tf_record_iterator(tf_filename)):
        dataset = dict()
        tf_example = tf.train.Example.FromString(example)
        tf_seq_example = tf.train.SequenceExample.FromString(example)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, choices=['val', 'test', 'train', 'train_neg'], default='val')
    ...
    if args.data == 'val':
        ...
        convert_train_data()
    elif args.data == 'train_neg':
        convert_neg_train_data()
    else:
        convert_test_data()
